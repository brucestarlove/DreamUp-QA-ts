...
- [x] Implement LLM evaluation function:
...
- [x] Combine heuristic and LLM scores:
  - Weight heuristic score (e.g., 0.6) and LLM confidence-weighted score (e.g., 0.4)
  - Or use LLM score as adjustment to heuristic
- [x] Include LLM responses in output JSON (when enabled)
- [x] Fall back to heuristic-only if LLM fails

### 5.4 Evaluation Prompts & Caching
- [x] Design concise prompts to reduce token usage
- [x] Implement basic LLM response caching:
...
- [x] Support `--llm` flag to enable/disable LLM evaluation
- [x] Support `--model` flag to override model selection

### 5.5 Cost Management
- [x] Track token usage per run via Stagehand metrics API:
...
- [x] Log token costs to output JSON
- [x] Use cheaper `executionModel` for non-reasoning actions (tool steps)
- [x] Implement cost-aware model selection
- [x] Persist cost metrics in session directory for dashboard visualization
